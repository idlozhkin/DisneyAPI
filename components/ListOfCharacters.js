...
export default function ListOfCharacters({ navigation, route }) {
  ...
  const addGroup = (keyText) => {
    if (!route.params.map((item) => item.key[0] === keyText).includes(true)) {
      route.params.push({ key: [keyText], characters: [] });
    }
  };
  ...
  const deleteCharacterFromList = (key) => {
    route.params.filter((obj) => obj.key[0] === keyValue)[0].characters =
      route.params
        .filter((obj) => obj.key[0] === keyValue)[0]
        .characters.filter((character) => character.key !== key);
    setCharacterList(route.params.filter((obj) => obj.key[0] === keyValue)[0].character);
  };
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  navigation.navigate("Character", [item, route.params]);
                }}
              >
                <Image
                  style={styles.image}
